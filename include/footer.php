<!-- footer section strats here -->
                     ...
                     <div class="footer-logo-section">
                         <div class="footer-top-section">
                             <div class="footer-logo">
                                 <a href="index.php">
                                     <img src="assets/image/Logdgo_1.png" alt="logo" width="100" style="background-color: white;padding: 10px;border-radius: 20px;">
                                 </a>
                             </div>
                             ...
                             <div class="payment-gateway">
                                 <h6 style="margin-top: 20px;">100% Secured Payment</h6>
                                 <div class="icons">
                                     <img src="assets/image/home1/2acfb6fb41f7fcb82c3230afdecff714.jpg" alt="phonepay" style="width: 10%;border-radius: 50px;">
                                     <img src="assets/image/home1/Frame-43-1.webp" alt="gpay" style="width: 10%;border-radius: 50px;">
                                     <img src="assets/image/home1/paytm.png" alt="paytm" style="width: 10%;border-radius: 50px;">
                                     <img src="assets/image/home1/Maestro_Logo.svg" alt="" style="width: 10%;border-radius: 50px;">
                                 </div>
                             </div>
                         </div>
                     </div>
                 ...
                 <div class="col-lg-2 col-md-4 col-sm-6 d-flex justify-content-lg-end ">
                     <div class="footer-widget">
                         ...
                         <ul class="widget-list">
                             ...
                             <li>
                                 <a href="refer.php">
                                     <a href="premium-member.php">Ivoric Premium</a>
                                 </a>
                             </li>
                         </ul>
                     </div>
                 ...
                 <div class="col-lg-4 col-md-6 col-sm-12 d-flex justify-content-lg-end justify-content-md-end">
                     <div class="footer-widget">
                         ...
                         <div class="footer-contact-section">
                             ...
                             <p>Mail Us - <span> ivoriclifestyle@gmail.com</span></p>
                         </div>
                     </div>
                 </div>
             </div>
    ...
    background:#222; color:#fff; padding:10px 20px; border-radius:6px;"></div>
     ...
     <script>
         ...
         function add_to_wishlist(product_id, user_id) {
             $.post("ajax.php", {
                 ...
                 try {
                     ...
                     if (typeof window.updateWishlistBadge === 'function' && typeof data.wishlist_count !== 'undefined') {
                         window.updateWishlistBadge(data.wishlist_count);
                     }
                 } catch (e) {
                     console.error('Invalid JSON from ajax.php (wishlist):', resp, e);
                     showToast('Server error. Try again.', 'error');
                 }
             }, 'json').fail(function(xhr, status, err) {
                 console.error('AJAX error (wishlist):', status, err, xhr.responseText);
                 showToast('Server error. Try again.', 'error');
             });
         }
         ...
         function delete_wishlist_item(product_id, user_id) {
             $.post("ajax.php", {
                 ...
                 try {
                     ...
                     console.error('Invalid JSON from ajax.php (delete wishlist):', resp, e);
                     showToast('Server error. Try again.', 'error');
                 }
             }, 'json').fail(function(xhr, status, err) {
                 console.error('AJAX error (delete wishlist):', status, err, xhr.responseText);
                 showToast('Server error. Try again.', 'error');
             });
         }

         // ---------------- Cart ----------------
         function addtocart(product_id, product_name, product_image, product_price, url, product_baseprice, size) {
             // try common selectors used across pages:
             var product_quantity = 1;
             if ($('#qty').length) {
                 product_quantity = parseInt($('#qty').val()) || 1;
             } else if ($('.quantity__input').length) {
                 product_quantity = parseInt($('.quantity__input').val()) || 1;
             } else if (typeof size === 'object' && size !== null && size.product_quantity) {
                 // defensive: in case caller passes an object accidentally
                 product_quantity = parseInt(size.product_quantity) || 1;
             }
             ...
             $.post("ajax.php", {
                 ...
                 try {
                     ...
                     console.error('Invalid JSON from ajax.php (add to cart):', resp, e);
                     showToast('Server error. Try again.', 'error');
                 }
             }, 'json').fail(function(xhr, status, err) {
                 console.error('AJAX error (add to cart):', status, err, xhr.responseText);
                 showToast('Server error. Try again.', 'error');
             });
         }

        function buyNow(product_id, product_name, product_size, product_image, product_price, url, product_baseprice) {
            if (!product_size) {
                showToast("Please select a size first!", "error");
                return;
            }
            
            let product_quantity = $('.quantity__input').val() || 1;
        ...
        //  function buyNow(id, name, size, image, price, url, base_price) {
        //      if (!size) {
        //          showToast("Please select a size first!", "error");
        //          return;
        //      }
        //      window.location.href = `checkout-page.php?product_id=${id}&name=${encodeURIComponent(name)}&image=${image}&price=${price}&url=${url}&base_price=${base_price}&size=${size}`;
        //  }
