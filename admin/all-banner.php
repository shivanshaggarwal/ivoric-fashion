...
<head>
    ...
    if (isset($_GET['type']) && $_GET['type'] != '') {
        ...
        if ($type == 'delete') {
            $id = get_safe_value($con, $_GET['id']);
            $select_sql = "SELECT image FROM banner WHERE id='$id'";
            ...
            if ($result) {
                ...
                $uploadsDirectory = '../media/banner/';
                $filePath = $uploadsDirectory . $filename;

                // Check if the file exists before attempting to delete it
                if (file_exists($filePath)) {
                    unlink($filePath); // Delete the file
                    echo "banner and image file deleted successfully";
                } else {
                                ...
                                <div class="card-body">
                                    <div class="title-header option-title">
                                        ...
                                        <form class="d-inline-flex">
                                            <a href="add-banner.php" class="align-items-center btn btn-theme d-flex">
                                                <i data-feather="plus-square"></i>Add Banner
                                            </a>
                                        </form>
                                    </div>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <div class="table-image">
                                                                    <?php
                                                                    echo "<a target='_blank' href='" . "../media/banner/" . $row['image'] . "'><img width='50px' src='" . "../media/banner/" . $row['image'] . "'/></a>";
                                                                    ?>
                                                                </div>
                                                            </td>
